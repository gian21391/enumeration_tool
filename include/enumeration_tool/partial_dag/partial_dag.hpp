...
namespace percy
{
    ...
    class partial_dag
    {
        ...
        public:
            ...
            bool operator==(const partial_dag &b) const {
              if (vertices == b.vertices) return true;
              return false;
            }
            ...
            void initialize_dfs_sequence()
            {
              ...
              for (auto item : vertices[starting_node]) {
                if (item != 0) initialize_dfs_sequence_impl(item - 1, visited_nodes);
              }
            }
            ...
            {
              auto insertion_result = visited_nodes.insert(starting_node);
              if (insertion_result.second) dfs_sequence.emplace_back(starting_node);
              for (auto item : vertices[starting_node]) {
                if (item != 0) initialize_dfs_sequence_impl(item - 1, visited_nodes);
              }
            }

            std::vector<unsigned>& get_dfs_sequence()
            {
              return dfs_sequence;
            }

            const std::vector<unsigned>& get_dfs_sequence() const
            {
              return dfs_sequence;
            }
            ...
            void foreach_vertex_dfs_call_on_zero(Fn&& fn) const
            {
              ...
              for (int i = 0; i < vertices[starting_node].size(); i++) {
                ...
                if (index < 0) fn({starting_node, i}, -1);
                else foreach_vertex_dfs_call_on_zero(index, fn);
              }
            }
            ...
            void foreach_vertex_dfs(unsigned starting_node, Fn&& fn) const
            {
              ...
              for (; i < dfs_sequence.size(); i++) {
                if (dfs_sequence[i] == starting_node) break;
              }
              for (; i < dfs_sequence.size(); i++) {
            ...
            const std::vector<int>& get_last_vertex() const
            {
              return vertices[vertices.size() - 1];
            }

            int get_last_vertex_index() const
            {
              return vertices.size() - 1;
            }

            const std::vector<int>& get_vertex(int v_idx) const
            {
            ...
            [[nodiscard]] std::vector<std::vector<int>>::size_type nr_vertices() const
            {
  ...
  struct reverse_it_state : public it_state {
    ...
    inline void begin(const partial_dag* ref) { it_state::end(ref); it_state::prev(ref);}
    inline void end(const partial_dag* ref) { it_state::begin(ref); it_state::prev(ref);}
  };
